        ...
        if (!using_stdin) {
                    ...
                    for (i = 0; i < nb_output_streams; i++) {
                        ...
                        if (ost->graph) {
                            if (time < 0) {
                                ...
                                LOGE( "Command reply for stream %d: ret:%d res:%s\n", i, ret, buf);
                            } else {
            ...
            if (key == 'd' || key == 'D'){
                ...
                if(key == 'D') {
                    ...
                    if(scanf("%d", &debug)!=1)
                        LOGE("error parsing debug value\n");
                for(i=0;i<nb_input_streams;i++) {
...
LOGD( "main(): reset_static_variables()" );
    reset_static_variables();

    OptionsContext o = { 0 };
    int64_t ti;

LOGD( "main(): reset_options()" );
    reset_options(&o, 0);

LOGD( "main(): av_log_set_flags(AV_LOG_SKIP_REPEATED)" );
...
LOGD( "main(): av_log_set_flags(AV_LOG_SKIP_REPEATED)" );
    ...
    if(argc>1 && !strcmp(argv[1], "-d")){
        run_as_daemon=1;
LOGD( "main(): Disabling logs" );
        av_log_set_callback(log_callback_null);
        argc--;
        argv++;
    }

LOGD( "main(): avcodec_register_all()" );
    avcodec_register_all();
#if CONFIG_AVDEVICE
LOGD( "main(): avdevice_register_all()" );
    avdevice_register_all();
#endif
#if CONFIG_AVFILTER
LOGD( "main(): avfilter_register_all_unsafe" );
    avfilter_register_all_unsafe();
#endif
LOGD( "main(): av_register_all()" );
    av_register_all();
LOGD( "main(): avformat_network_init()" );
    avformat_network_init();

LOGD( "main(): show_banner()" );
    show_banner(argc, argv, options);

LOGD( "main(): term_init()" );
    term_init();

LOGD( "main(): parse_options()" );
    /* parse options */
    parse_options(&o, argc, argv, options, opt_output_file);
LOGD( "main(): nb_output_files=%d nb_input_files=%d", nb_output_files, nb_input_files );
...
LOGD( "main(): transcode()" );
    ...
    if (do_benchmark) {
        ...
        LOGD("bench: utime=%0.3fs maxrss=%ikB\n", ti / 1000000.0, maxrss);
    }
